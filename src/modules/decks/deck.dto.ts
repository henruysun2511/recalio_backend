...
export class QueryDeckDto extends SearchDto {
  ...
  @IsOptional()
  @IsString({ message: 'userId phải là chuỗi kí tự' })
  userId?: string;
...
export class CreateDeckDto {
  ...
  @MinLength(DECK_CONSTANTS.NAME_MIN_LENGTH, {
    message: 'Tên deck không được để trống',
  })
  @MaxLength(DECK_CONSTANTS.NAME_MAX_LENGTH, {
    message: 'Tên deck không được quá 200 kí tự',
  })
  ...
  @MaxLength(DECK_CONSTANTS.DESC_MAX_LENGTH, {
    message: 'Mô tả không được quá 2000 kí tự',
  })
  @Transform(({ value }) => value?.trim())
  description?: string;
  ...
  @MaxLength(DECK_CONSTANTS.TAG_MAX_LENGTH, {
    each: true,
...
export class DeckResponseDto {
  ...
  @ApiPropertyOptional({ type: DeckProgressDto })
