...
export class NotificationCron {
    ...
    async createStudyReminders() {
        ...
        for (const userId of userIds) {
            const exists = await this.repo.hasRecentNotification(userId, NotificationType.STUDY_REMINDER, today);
            if (exists) continue;
    ...
    async createCardsDueNotifications() {
        ...
        const created: string[] = [];
        for (const userId of userIds) {
            const exists = await this.repo.hasRecentNotification(userId, NotificationType.CARDS_DUE, since);
            if (exists) continue;
        ...
        if (created.length) {
            this.logger.log(`Created CARDS_DUE for ${created.length} users (${userIds.length} have due cards)`);
        }
    }
    ...
    async sendPendingEmails() {
        ...
        const grouped = new Map<string, { ids: string[]; email: string; name: string; items: { type: string; title: string; body: string | null }[] }>();

        for (const n of notifications) {
            const key = n.user.id;
            if (!grouped.has(key)) {
                grouped.set(key, { ids: [], email: n.user.email, name: n.user.displayName, items: [] });
            }
        ...
        for (const [, group] of grouped) {
            const itemsHtml = group.items.map((i) => `<li><b>${i.title}</b>${i.body ? `: ${i.body}` : ''}</li>`).join('');
        ...
        this.logger.log(`Queued ${notifications.length} email notifications to ${grouped.size} users`);
    }
}
