...
export class NotificationService {
  ...
  async updateSettings(userId: string, dto: UpdateSettingsDto) {
    const updated = await this.repo.upsertSettings(userId, dto);
  ...
  async createNotification(adminId: string, dto: CreateNotificationDto) {
    ...
    if (
      resolvedChannel === NotificationChannel.EMAIL &&
      !this.EMAIL_ELIGIBLE_TYPES.has(type)
    ) {
    ...
    if (resolvedUserId) {
      ...
      this.logger.log(
        `Admin ${adminId}: queued notification to user ${resolvedUserId}`,
      );
      return { queued: true };
    }
