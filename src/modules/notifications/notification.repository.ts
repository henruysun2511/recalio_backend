...
import { Prisma, NotificationType, NotificationChannel, CardState } from '@prisma/client';
...
export class NotificationRepository {
    ...
    async upsertSettings(userId: string, data: Prisma.NotificationSettingUpdateInput) {
        return this.prisma.notificationSetting.upsert({
            where: { userId },
            update: data,
            create: { userId, ...data } as any,
            select: { emailEnabled: true, pushEnabled: true, studyReminder: true, reminderTime: true },
        });
    }

    async findMany(userId: string, where?: Prisma.NotificationWhereInput, page = 1, limit = 20) {
    ...
    async getEligibleUserIds(channel: string) {
        const channelField = channel === NotificationChannel.EMAIL ? ('emailEnabled' as const) : ('pushEnabled' as const);
        ...
        const settingsMap = new Map(settings.map((s) => [s.userId, s[channelField]]));
    ...
    async findUsersWithDueCards() {
        const users = await this.prisma.user.findMany({
            where: {
                ...
                OR: [
                    { notifSetting: null },
                    { notifSetting: { emailEnabled: true } },
                ],
            } as any,
            select: { id: true },
        });
    ...
    async hasRecentNotification(userId: string, type: NotificationType, since: Date) {
    ...
    async findUnsentEmails() {
        const notifications = await this.prisma.notification.findMany({
            where: {
                ...
                emailSentAt: null,
                type: { in: [NotificationType.STUDY_REMINDER, NotificationType.CARDS_DUE, NotificationType.SYSTEM] },
        ...
        const disabled = new Set(settings.filter((s) => !s.emailEnabled).map((s) => s.userId));
        ...
        const disabledNotifIds = notifications.filter((n) => disabled.has(n.user.id)).map((n) => n.id);
