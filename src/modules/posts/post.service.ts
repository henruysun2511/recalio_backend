...
export class PostService {
  ...
  async findAll(currentUserId: string, dto: PostQueryDto) {
    const { posts, total } = await this.repo.findAll(currentUserId, dto);
    return paginate(posts, total, dto);
  }
  ...
  async update(userId: string, postId: string, dto: UpdatePostDto) {
    ...
    if (dto.deckIds !== undefined) {
      const deckIds = dto.deckIds.map((id, i) => ({
        deckId: id,
        orderIndex: i,
      }));
  ...
  async findAllAdmin(dto: PostQueryDto) {
    const { posts, total } = await this.repo.findAllAdmin(dto);
    return paginate(posts, total, dto);
  }
