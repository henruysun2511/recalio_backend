...
import {
  UpdateDeckSettingDto,
  DeckSettingResponseDto,
} from './deck-setting.dto';
...
export class DeckSettingController {
  ...
  @SwaggerDoc({
    summary: 'Get deck setting',
    responseType: DeckSettingResponseDto,
  })
  async get(
  ...
  @SwaggerDoc({
    summary: 'Update deck setting',
    bodyType: UpdateDeckSettingDto,
    responseType: DeckSettingResponseDto,
  })
  async update(
    @CurrentUser('id') userId: string,
    @Param('deckId') deckId: string,
    @Body() dto: UpdateDeckSettingDto,
  ) {
