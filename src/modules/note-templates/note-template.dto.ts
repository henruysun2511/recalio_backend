...
export class CreateCardTemplateDto {
  ...
  @MaxLength(NOTE_TEMPLATE_CONSTANTS.CARD_TEMPLATE_NAME_MAX_LENGTH, {
    message: 'Tên card template không được quá 100 kí tự',
  })
  ...
  @MaxLength(NOTE_TEMPLATE_CONSTANTS.HTML_MAX_LENGTH, {
    message: 'frontHtml không được quá 5000 kí tự',
  })
  @Transform(({ value }) => value?.trim())
  frontHtml: string;
  ...
  @MaxLength(NOTE_TEMPLATE_CONSTANTS.HTML_MAX_LENGTH, {
    message: 'backHtml không được quá 5000 kí tự',
  })
  @Transform(({ value }) => value?.trim())
  backHtml: string;
  ...
  @MaxLength(NOTE_TEMPLATE_CONSTANTS.CSS_MAX_LENGTH, {
    message: 'css không được quá 5000 kí tự',
  })
  @Transform(({ value }) => value?.trim())
  css?: string;
}
...
export class CreateNoteTemplateDto {
  ...
  @MaxLength(NOTE_TEMPLATE_CONSTANTS.NAME_MAX_LENGTH, {
    message: 'Tên template không được quá 100 kí tự',
  })
  @Transform(({ value }) => value?.trim())
  name: string;
  ...
  @ArrayMaxSize(NOTE_TEMPLATE_CONSTANTS.FIELD_NAMES_MAX, {
    message: 'Không được quá 20 field',
  })
  @IsString({ each: true, message: 'Mỗi field name phải là chuỗi kí tự' })
  @MinLength(1, { each: true, message: 'Tên field không được để trống' })
  @MaxLength(NOTE_TEMPLATE_CONSTANTS.FIELD_NAME_MAX_LENGTH, {
    each: true,
    message: 'Tên field không được quá 50 kí tự',
  })
  @Transform(({ value }) => value?.map((v: string) => v.trim()))
  fieldNames: string[];

  @ApiPropertyOptional({
    type: [CreateCardTemplateDto],
    description: 'Card templates to create with this note template',
  })
  @IsOptional()
