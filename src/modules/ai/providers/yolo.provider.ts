...
export class YoloProvider extends AiProvider {
    ...
    private readonly scriptPath = join(__dirname, '../../../../scripts/yolo_detect.py');
    ...
    async analyzeImage(imageBuffer: Buffer, _mimeType: string, _prompt: string): Promise<string> {
        ...
        return new Promise((resolve, reject) => {
            ...
            proc.on('close', (code) => {
                if (code !== 0) {
                    ...
                    if (stderr.includes('No module named') || stderr.includes('not found')) {
                        reject(new Error('YOLOv10 not installed. Run: pip install ultralytics pillow'));
                    } else {
            ...
            proc.on('error', (err) => {
                reject(new Error(`Python not found. Install Python 3 + pip install ultralytics pillow`));
            });
