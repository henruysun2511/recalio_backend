...
export class AiService {
  ...
  async generateRelatedNotes(
    dto: RelatedNotesDto,
  ): Promise<RelatedNotesResponseDto> {
    if (!this.provider.isConfigured()) throw AiError.aiNotConfigured();
  ...
  async processDocument(
  ...
  ): Promise<ProcessDocumentNoteDto[]> {
    if (!this.provider.isConfigured()) throw AiError.aiNotConfigured();
    ...
    if (pageCount > AI_CONSTANTS.MAX_PDF_PAGES) {
      throw new BadRequestException(
        `PDF vượt quá ${AI_CONSTANTS.MAX_PDF_PAGES} trang (hiện tại: ${pageCount})`,
      );
    }
  ...
  async detectImage(
    file: Express.Multer.File,
  ): Promise<DetectImageResponseDto> {
    if (!this.provider.isConfigured()) throw AiError.aiNotConfigured();

    const raw = await this.provider.analyzeImage(
      file.buffer,
      file.mimetype,
      AI_PROMPTS.DETECT_IMAGE,
    );
