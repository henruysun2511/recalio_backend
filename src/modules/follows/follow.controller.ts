...
export class FollowController {
    ...
    @SwaggerDoc({ summary: 'Follow a user', responseType: FollowUserDto, status: 201 })
    ...
    async getFollowingStatus(@CurrentUser('id') userId: string, @Param('id') id: string) {
        return this.service.getFollowingStatus(userId, id);
    }

    @Get(':userId/following')
    @ResponseMessage('Lấy danh sách đang follow thành công')
    @SwaggerDoc({ summary: 'List who a user is following', responseType: FollowUserDto, isArray: true })
    async getUserFollowing(@Param('userId') userId: string, @Query() dto: PaginationDto) {
        return this.service.getFollowing(userId, dto);
    }

    @Get(':userId/followers')
    @ResponseMessage('Lấy danh sách người theo dõi thành công')
    @SwaggerDoc({ summary: 'List followers of a user', responseType: FollowUserDto, isArray: true })
    async getUserFollowers(@Param('userId') userId: string, @Query() dto: PaginationDto) {
        return this.service.getFollowers(userId, dto);
    }

    @Get('following')
    @ResponseMessage('Lấy danh sách đang follow thành công')
    @SwaggerDoc({ summary: 'List who I am following', responseType: FollowUserDto, isArray: true })
    async getFollowing(@CurrentUser('id') userId: string, @Query() dto: PaginationDto) {
        return this.service.getFollowing(userId, dto);
    }

    @Get('followers')
    @ResponseMessage('Lấy danh sách người theo dõi thành công')
    @SwaggerDoc({ summary: 'List my followers', responseType: FollowUserDto, isArray: true })
    async getFollowers(@CurrentUser('id') userId: string, @Query() dto: PaginationDto) {
        return this.service.getFollowers(userId, dto);
    }
}
