...
export class StudySessionService {
    ...
    async end(userId: string, id: string) {
        ...
        this.logger.log(`User ${userId}: ended session ${id}`);
        return updated;
    }
    ...
    async getById(userId: string, id: string): Promise<SessionResponseDto> {
        ...
        return { id: session.id, deckId: session.deckId, mode: session.mode, startedAt: session.startedAt, endedAt: session.endedAt, stats };
    }

    async list(userId: string, dto: ListSessionQueryDto) {
        const page = dto.page ?? 1;
        const limit = dto.limit ?? SESSION_CONSTANTS.DEFAULT_LIMIT;
        const { items, total } = await this.repo.findByUser(userId, dto.deckId, page, limit);
