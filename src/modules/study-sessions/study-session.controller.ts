...
export class StudySessionController {
    ...
    @SwaggerDoc({ summary: 'List study sessions', responseType: SessionResponseDto, isArray: true })
    async list(@CurrentUser('id') userId: string, @Query() dto: ListSessionQueryDto) {
        return this.service.list(userId, dto);
    }

    @Get(':id')
    @ResponseMessage('Lấy chi tiết phiên học')
    @SwaggerDoc({ summary: 'Get session detail', responseType: SessionResponseDto })
    ...
    async getReviewLogs(@CurrentUser('id') userId: string, @Param('id') id: string, @Query() dto: PaginationDto) {
