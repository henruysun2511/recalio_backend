...
export class NoteProcessor extends WorkerHost {
  ...
  async process(job: Job<ProcessAudioData>) {
    ...
    try {
      ...
      this.logger.log(
        `Audio resolved for note ${noteId}: "${word}" — ${audioUrl ?? 'no audio'}`,
      );
    } catch (err) {
      this.logger.error(
        `Audio failed for note ${noteId} "${word}": ${(err as Error).message}`,
      );
      throw err;
    }
  }
  ...
  onFailed(job: Job, err: Error) {
    this.logger.error(
      `Job ${job.id} failed: "${job.data.word}" — ${err.message}`,
    );
  }
}
